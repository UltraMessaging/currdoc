...
//	private static String pcid = "";
	...
	public static boolean flightsz_user_set = false;
	public static int flightsz_src_type = 0;
...
;
	...
	public static void main(String[] args)
	{
		...
		Getopt gopt = new Getopt("umqsrc", args, "+A:B:c:C:ef:hil:L:m:M:nNP:R:s:vX:Y:", longopts);
		int c = -1;
		int msglen = 25;
		long bytes_sent = 0;
		boolean error = false;
					...
					case 'Y':
						...
						byte[] index = gopt.getOptarg().getBytes("ASCII");
							try {
								...
								System.err.println("Error setting UMQ index for broker source: " + ex.toString());
								System.exit(1);
							}
						break;		
					default:
		...
		{
			try {
				cattr.setProperty("broker", broker);	
			}
			catch (LBMRuntimeException ex) {
				System.err.println("Error setting broker address: " + ex.toString());
			}
			try {
				sattr.setProperty("transport", "broker");	
			}
			catch (LBMRuntimeException ex) {
				System.err.println("Error setting broker transport: " + ex.toString());
			}
		}
		if (appsets != null)
		{
        ...
        boolean isBrokerTransport = false;
		try 
		{
            isBrokerTransport = sattr.getValue("transport").equalsIgnoreCase("broker");
		...
		if (!isBrokerTransport && (appsets == null || appsets.compareTo("") == 0)) {
			System.err.println("Broker not set and ULB application set not set. Exiting.");
			System.exit(1);
		}
		else
		{
			if (isBrokerTransport) {
				flightsz_src_type = LBM.FLIGHT_SIZE_TYPE_UMQ;
				if (flightsz_user_set) {
					sattr.setProperty("umq_flight_size", Integer.toString(flightsz));
				}
				else {
					try {
						flightsz = Integer.parseInt(sattr.getValue("umq_flight_size"));
					} catch (NumberFormatException e) {
						System.err.println("Error getting flight size value: " + e.toString());
						System.exit(1);
			...
			if (appsets != null && appsets.compareTo("") != 0)
			{
				...
				flightsz_src_type = LBM.FLIGHT_SIZE_TYPE_ULB;
				...
				if (flightsz_user_set) {
					sattr.setProperty("umq_ulb_flight_size", Integer.toString(flightsz));
				}
				else {
					try {
						flightsz = Integer.parseInt(sattr.getValue("umq_ulb_flight_size"));
					} catch (NumberFormatException e) {
						System.err.println("Error getting flight size value: " + e.toString());
						System.exit(1);
					} catch (LBMException e) {
		...
		if (stats_sec > 0)
		{
			try
			{
				stats = new UMQSrcStatsTimer(ctx, src, flightsz_src_type, stats_sec * 1000, objRec);
			}
			catch (LBMException ex)
			{
	...
	public int onSourceEvent(Object arg, LBMSourceEvent sourceEvent)
	{
		switch (sourceEvent.type())
		{
			...
			case LBM.SRC_EVENT_WAKEUP:
				if (_verbose > 0) {
					System.out.println("Source wakeup event flags 0x" + Integer.toHexString(sourceEvent.sourceWakeupInfo().flags()) + "\n");
				}
				blocked = false;
				break;
			...
			case LBM.SRC_EVENT_UMQ_MESSAGE_STABLE_EX:
				...
				if(_verbose > 0) {
					System.out.print("UMQ "
							...
							+ "] (cd "
							+ Long.toHexString(((Long)q_ack.clientObject()).longValue())
							+ ") stable. Flags "
...
class UMQSrcStatsTimer extends LBMTimer
{
	...
	public UMQSrcStatsTimer(LBMContext ctx, LBMSource src, int src_type, long tmo, LBMObjectRecyclerBase objRec) throws LBMException
	{
		super(ctx, tmo, null);
		_src = src;
		_tmo = tmo;
	...
	private void print_stats()
	{
		try
		{
			...
			int inflight = _src.getInflight(_src_type);
			...
			switch (stats.type())
			{
				...
				case LBM.TRANSPORT_STAT_LBTSMX:
					System.out.println("LBT-SMX, clients "
								+ stats.numberOfClients()
								+ ", sent " 
								+ stats.messagesSent()
								+ "/"
				...
				case LBM.TRANSPORT_STAT_BROKER:
					System.out.println(stats.source()
								+ " sent "
								+ stats.messagesSent()
								+ "/"
								+ stats.bytesSent()
