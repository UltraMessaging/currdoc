...
char usage[] =
"Usage: lbmhtrcv [options] <patterns_file\n"
"Where 'patterns_file' is a simple text file, supplied as standard input,\n"
...
void print_bw(FILE *fp, struct timeval *tv, unsigned int msgs, unsigned int bytes, int lost)
{
	...
	if ((rx_msg_count != 0) || (otr_msg_count != 0))
		fprintf(fp, "%-5.4g secs.  %-5.4g %cmsgs/sec.  %-5.4g %cbps [RX: %lu][OTR: %lu]",
			sec, mps, scale[msg_scale_index], bps, scale[bit_scale_index], (unsigned long) rx_msg_count, (unsigned long) otr_msg_count);
	else
	...
	if (lost != 0 || unrec_count != 0 || burst_loss != 0){
		fprintf(fp, " [%u pkts lost, %lu msgs unrecovered, %lu bursts]",
			    lost, (unsigned long) unrec_count, (unsigned long) burst_loss);
	}
