...
"application that receives messages from a given topic using a single\n"
"    hot-failover receiver."
;

char usage[] =
"Usage: lbmhfrcv [-AEhsvV] [-c filename] [-r msgs] [-U losslev] topic\n"
"       -A = display messages as ASCII text\n"
"       -c filename = Use LBM configuration file filename.\n"
"                     Multiple config files are allowed.\n"
"                     Example:  '-c file1.cfg -c file2.cfg'\n"
"       -d, --msec-delay=NUM        Implements a number of milliseconds sleep per message received\n"
...
int rcv_handle_msg(lbm_rcv_t *rcv, lbm_msg_t *msg, void *clientd)
{
 ...
 char * file_msg      = "lbmhfrcv::rcv_handle_msg";
 char * hf_rcv_ex_msg = "UMP receiver registration successful for a store (extended form)";
 char * hf_reg_msg    = "UMP receiver notification of registration completion";
  ...
  case LBM_MSG_UME_REGISTRATION_SUCCESS_EX:
    printf( "%s %s [%s][%s]\n", file_msg, hf_rcv_ex_msg, msg->topic_name, msg->source );
    break;

  case LBM_MSG_UME_REGISTRATION_COMPLETE_EX:
    printf( "%s %s [%s][%s]\n", file_msg, hf_reg_msg, msg->topic_name, msg->source );
    break;

	 default:
		  printf( "Unhandled receiver event [%d] from source [%s] topic [%s]. Refer to https://ultramessaging.github.io/currdoc/doc/example/index.html#unhandledcevents for a detailed description.\n", msg->type, msg->source, msg->topic_name);
		  break;
	}
	/* LBM automatically deletes the lbm_msg_t object unless we retain it. */
	return 0;
...
const char * option_string = "Ac:d:Ehr:sSvV";
