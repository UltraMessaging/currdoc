...
"\n"
"Alternate usage: lbmspike [-dhLn] [-B bghumms] [-c filename] [-l len] [-M msgs] [-r rate/pct] [-v recovms] [topic]\n"
...
int handle_rcvc_msg(lbm_rcv_t *rcv, lbm_msg_t *msg, void *clientd)
{
	...
	case LBM_MSG_DATA:
		...
		case MAGIC_SETUP:
			...
			seqno0  = arg4;
			nogapnext = seqno0;
			printf("Expecting %u messages of length %lu\n",
				msgs, (unsigned long) msglen);
			if (starttv.tv_sec != 0) {
				starttv.tv_sec = 0;
				starttv.tv_usec = 0;
			}
...
printf("A");
	...
	if (roleRcv) {	/* Spike receiver role */
		...
		snprintf(cmsg, sizeof(cmsg), "%d %lu %d %lu %d",
			MAGIC_SETUP, (unsigned long) msglen, msgs, recovms, setupms/bghumms);
		...
		for (i=0; i<msgs; i++) {
			...
			snprintf(dmsg, sizeof(dmsg), "%d %ld %ld",
				MAGIC_SPIKE, (long int) mts[i].tv_sec, (long int) mts[i].tv_usec);
			if (strlen(dmsg)+1 > msglen) {
