...
"application that uses Smart Source sends to a given topic. Understands persistence."
;
...
void print_stats(FILE *fp, lbm_ssrc_t *ssrc)
{
	...
	/* Retrieve smart source transport statistics */
	if (lbm_ssrc_retrieve_transport_stats(ssrc, &stats) == LBM_FAILURE) {
		fprintf(stderr, "lbm_ssrc_retrieve_stats: %s\n", lbm_errmsg());
		exit(1);
	}
	lbm_ssrc_get_inflight(ssrc, LBM_FLIGHT_SIZE_TYPE_UME, &inflight, NULL, NULL);
	switch (stats.type) {
...
/* Smart Source event handler callback (passed into lbm_ssrc_create()) */
	...
	case LBM_SRC_EVENT_UME_REGISTRATION_ERROR:
		{
			...
			printf("Error registering smart source with UME store: %s\n", errstr);
		}
		break;
...
/* Timer callback to handle periodic display of smart source statistics */
...
int handle_force_reclaim(const char *topic, lbm_uint_t sqn, void *clientd)
{
	...
	if (tsp == NULL) {
		fprintf(stderr,"WARNING: smart source for topic \"%s\" forced reclaim 0x%x\n", topic, sqn);
	} else {
		current_tv(&endtv);
		...
		force_reclaim_total++;
		if (secs > 5.0) {
			fprintf(stderr,"WARNING: smart source for topic \"%s\" forced reclaim. Total %d.\n", topic, force_reclaim_total);
			current_tv(&nowtv);
...
int check_ume_store_config(lbm_src_topic_attr_t *tattr, lbm_context_attr_t *cattr, struct Options *opts)
{
   ...
   printf("Using smart source Session ID %"PRIu64".  Any per-store registration IDs specified will be ignored.\n", session_id);
...
int main(int argc, char **argv)
{
 	...
 	}
	{
		...
		/* Retrieve current smart source topic settings */
	...
	for (count = 0; count < opts->msgs; ) {
		...
		for (i = 0; i < msgs_per_ivl; i++)
		{
			...
			blocked = 1;
			/* Send message using allocated smart source */
			if (lbm_ssrc_send_ex(ssrc, message, opts->msglen, xflag, &exinfo) == LBM_FAILURE) {
				if (lbm_errnum() == LBM_EWOULDBLOCK)
				{
					if (opts->verbose)
					{
						printf("Send returned EWOULDBLOCK due to flight size. Waiting...\n");
					}
					while (blocked)
					{
						SLEEP_MSEC(100);
				...
				if (lbm_errnum() == LBM_EUMENOREG)
				{
					...
					if (opts->verbose)
					{
						printf("lbm_ssrc_send_ex: %s errnum: %d\n", lbm_errmsg(), lbm_errnum());
					}
				...
				{
					fprintf(stderr, "lbm_ssrc_send_ex: %s errnum: %d\n", lbm_errmsg(), lbm_errnum());
					exit(1);
				}
			}
			blocked = 0;
	...
	if (opts->deregister) {
		printf("Deregistering smart source\n");
		lbm_ssrc_ume_deregister(ssrc);
	}
