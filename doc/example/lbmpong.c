...
"application that measures round trip message latency."
;

char usage[] =
"Usage: lbmpong [-ChIqRv] [-c filename] [-i msgs] [-l len] [-M msgs] [-P msec] [-r rate/pct] [-s seed] [-t secs] [-T topic] id\n"
...
void print_latency(FILE *fp, struct timeval *tv, size_t count)
{
	...
	latency = rtt / 2.0;
	fprintf(fp, "Elapsed time %.04g secs. %lu messages (RTTs). %.04g msec RTT, %.04g msec latency\n", sec,
			(unsigned long) count, rtt, latency);
	fflush(fp);
}
...
int rcv_handle_msg(lbm_rcv_t *rcv, lbm_msg_t *msg, void *clientd)
{
	switch (msg->type) {
	case LBM_MSG_DATA:
		if ((use_mim == 0) && (transport == LBM_SRC_TOPIC_ATTR_TRANSPORT_LBTSMX)) {
			if (lbm_src_buff_acquire(src, (void **)&message, msglen, 0) == LBM_FAILURE) {
		...
		if (use_mim) {
			...
			} else if (lbm_src_send(src, message, msglen, LBM_MSG_FLUSH | LBM_SRC_NONBLOCK) == LBM_FAILURE) {
				fprintf(stderr, "lbm_src_send: %s\n", lbm_errmsg());
				exit(1);
			}
		}
		if(rtt_ignore > 0) rtt_ignore--;
		if (verbose && (msg->hr_timestamp.tv_sec != 0)) {
			fprintf(stdout, "RX HR_TS: " TIME_FMT " for SQN %u\n", (uint64_t)msg->hr_timestamp.tv_sec,
				(uint64_t)msg->hr_timestamp.tv_nsec, msg->sequence_number);
		}
		break;
	case LBM_MSG_BOS:
	...
	case LBM_SRC_EVENT_TIMESTAMP:
		{
			...
			if (verbose) {
				fprintf(stdout, "TX HR_TS: " TIME_FMT " for SQN %u\n", (uint64_t)info->hr_timestamp.tv_sec,
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, OptionString)) != EOF) {
		...
		case 'T':
			if (strlen(optarg) > (sizeof(src_topic_name) - 10)) {
				fprintf(stderr, "-T value longer than %lu chars\n",
						(unsigned long) (sizeof(src_topic_name) - 10));
				errflag++;
			}
	...
	if (!(msglen >= sizeof(struct timeval))) {
		fprintf(stderr, "Message length (%lu) must be >= sizeof(struct timeval) (%lu)\n", (unsigned long) msglen, (unsigned long) sizeof(struct timeval));
		exit(1);
	}
	if (use_mim && eventq == 0) {
	...
	if(rtt_measure && rtt_collect) {
		...
		rtt_data = (double *) malloc(msgs * sizeof(rtt_data[0]));
	}
	...
	if (ping) {
		/* Pinger: */
		fprintf(stderr, "Sending %d %lu byte messages, pausing %d msec between\n", msgs, (unsigned long) msglen, msecpause);
	...
	/* Note: With SMX, it is possible to allocate directly from the share memory. This is optional,
	 * and the developer may choose not to use this optimization for convenience.
	 *
	 * Get and setup message buffer needed by pinger and ponger -- use special handling
	 * for the pinger and for SMX -- as we will be using this right away
	 */
	if (ping && (use_mim == 0) && (transport == LBM_SRC_TOPIC_ATTR_TRANSPORT_LBTSMX)) {
		if (lbm_src_buff_acquire(src, (void **)&message, msglen, 0) == LBM_FAILURE) {
			fprintf(stderr, "lbm_src_buff_acquire: %s\n", lbm_errmsg());
			exit(1);
		}
	...
	if (ping) {
		...
		if (use_mim) {
			...
			if (transport == LBM_SRC_TOPIC_ATTR_TRANSPORT_LBTSMX) {
				lbm_src_buffs_complete(src);
			} else if (lbm_src_send(src, message, msglen, LBM_MSG_FLUSH) == LBM_FAILURE) {
				fprintf(stderr, "lbm_src_send: %s\n", lbm_errmsg());
				exit(1);
			}
		}
	}
	if ((ping == 0) && (use_mim == 0) && (transport == LBM_SRC_TOPIC_ATTR_TRANSPORT_LBTSMX)) {
		/* In the case of the ponger using SMX, we have a memory leak -- the message buffer won't
		   be used as the special handling will be getting it from shared memory. For now, let's
		   just free it.  We can fix the logic later to *not* malloc it in the first place.
		 */
		free(message);
		message = NULL;
	}
