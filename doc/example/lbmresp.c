...
"application that receives request messages on a single topic and\n"
...
int rcv_handle_immediate_msg(lbm_context_t *ctx, lbm_msg_t *msg, void *clientd)
{
	...
	case LBM_MSG_REQUEST:
		...
		if (verbose) {
			printf("IM Request [%s][%u], %lu bytes%s\n",
				   msg->source, msg->sequence_number, (unsigned long) msg->len,
				   skipped ? " (ignored)"  : "");
			if (verbose > 1)
				dump(msg->data, msg->len);
...
int rcv_handle_msg(lbm_rcv_t *rcv, lbm_msg_t *msg, void *clientd)
{
	...
	case LBM_MSG_DATA:
		...
		if (verbose) {
			printf("[%s][%s][%u], %lu bytes\n",
				   msg->topic_name, msg->source, msg->sequence_number, (unsigned long) msg->len);
			if (verbose > 1) 
				dump(msg->data, msg->len);
		}
		break;
	...
	case LBM_MSG_REQUEST:
		...
		skipped = response != NULL;
		if (verbose) {
			printf("Request [%s][%s][%u], %lu bytes%s\n",
				   msg->topic_name, msg->source, msg->sequence_number,
				   (unsigned long) msg->len, skipped ? " (ignored)" : "");
			if (verbose > 1)
				dump(msg->data, msg->len);
		}
		if (!skipped)
...
int main(int argc, char **argv)
{
	...
	if ((response_buffer = malloc(response_len + sizeof(lbm_serialized_response_t) + 11)) == NULL) {
		fprintf(stderr, "could not allocate response buffer of size %lu bytes\n", (unsigned long) response_len);
		exit(1);
	}
	...
	while (1) {
		...
		if (response != NULL) {
			if (verbose) {
				if (!forward_requests) {
					printf("Sending response. %u response%s of %lu bytes%s (%lu total bytes).\n",
					 	   responses, (responses == 1 ? "" : "s"), (unsigned long) response_len, (responses == 1 ? "" : " each"), 
						   (unsigned long) (responses * response_len));
				}
				else {
