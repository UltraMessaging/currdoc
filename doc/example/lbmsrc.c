...
const char* OptionString = "c:d:j:hL:l:M:nN:P:R:s:vVG:";
...
struct Options {
	...
	unsigned int datagram_max_size;           /* Source datagram max size */
	...
	*/
	if (opts->datagram_max_size != 0) {
		if (opts->rm_protocol == 'M' || opts->rm_protocol == 'U') {
			printf("Sending with LBT-R%c source datagram max size %u\n",
		...
		case 'M':
			...
			if (lbm_context_attr_setopt(cattr, "transport_lbtrm_source_datagram_max_size", &opts->datagram_max_size, sizeof(opts->datagram_max_size)) != 0) {
				fprintf(stderr, "lbm_context_attr_setopt:transport_lbtrm_source_datagram_max_size: %s\n", lbm_errmsg());
				exit(1);
			}
			break;
		case 'U':
			/* Set transport attribute to LBT-RU */
			if (lbm_src_topic_attr_str_setopt(tattr, "transport", "LBTRU") != 0) {
				fprintf(stderr, "lbm_src_topic_str_setopt:transport: %s\n", lbm_errmsg());
				exit(1);
			}
			/* Set LBT-RU source datagram max size attribute */
			if (lbm_context_attr_setopt(cattr, "transport_lbtru_source_datagram_max_size", &opts->datagram_max_size, sizeof(opts->datagram_max_size)) != 0) {
				fprintf(stderr, "lbm_context_attr_setopt:transport_lbtru_source_datagram_max_size: %s\n", lbm_errmsg());
				exit(1);
			}
			break;
		case 'S':
			printf("Sending with Resolver source datagram max size %u\n", opts->datagram_max_size);
			/* Set Resolver source datagram max size attribute */
			if (lbm_context_attr_setopt(cattr, "resolver_source_datagram_max_size", &opts->datagram_max_size, sizeof(opts->datagram_max_size)) != 0) {
				fprintf(stderr, "lbm_context_attr_setopt:resolver_source_datagram_max_size: %s\n", lbm_errmsg());
				exit(1);
			}
			break;
		}
