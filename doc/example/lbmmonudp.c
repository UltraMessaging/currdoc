...
"application that receives LBM statistics and forwards as CSV over UDP."
;
...
#define LBMMONUDP_BROKER_SRC_V1 17
#define LBMMONUDP_BROKER_RCV_V1 18
...
#define LBMMONUDP_BROKER_SRC_VER LBMMONUDP_BROKER_SRC_V1
#define LBMMONUDP_BROKER_RCV_VER LBMMONUDP_BROKER_RCV_V1
...
rcv_statistics_cb(const void * AttributeBlock, const lbm_rcv_transport_stats_t * Statistics, void * ClientData)
{
	...
	switch (Statistics->type)
	{
		...
		case LBM_TRANSPORT_STAT_BROKER:
			format = LBMMONUDP_BROKER_RCV_VER;
			break;
	...
	switch (Statistics->type)
	{
		...
		case LBM_TRANSPORT_STAT_BROKER:
			snprintf(ptr,
					remaining_len,
					"%lu,%lu\n",
					preprocess_value(Statistics->transport.broker.msgs_rcved),
					preprocess_value(Statistics->transport.broker.bytes_rcved));
			break;
	}
...
src_statistics_cb(const void * AttributeBlock, const lbm_src_transport_stats_t * Statistics, void * ClientData)
{
	...
	switch (Statistics->type)
	{
		...
		case LBM_TRANSPORT_STAT_BROKER:
			format = LBMMONUDP_BROKER_SRC_VER;
			break;
	...
	switch (Statistics->type)
	{
		...
		case LBM_TRANSPORT_STAT_BROKER:
			snprintf(ptr,
					remaining_len,
					"%lu,%lu\n",
					preprocess_value(Statistics->transport.broker.msgs_sent),
					preprocess_value(Statistics->transport.broker.bytes_sent));
			break;
	}
	send_packet(buffer, strlen(buffer));
}
...
resolve_ipv4_address()
{
	...
	if (sscanf(optarg, "%3u.%3u.%3u.%3u", &d[0], &d[1], &d[2], &d[3]) == 4) {
		...
		if (strlen(optarg) >= sizeof(namestr)) {
			fprintf(stderr, "the domain name is too long\n");
			return (INADDR_NONE);
		}
		matchcount = sscanf(optarg, "%[A-Za-z0-9-.]", namestr);
		if (matchcount == 0) {
			fprintf(stderr, "the domain name is malformed\n");
			return (INADDR_NONE);
		}
		if (strlen(namestr) < 2) {
			fprintf(stderr, "the domain name is too short\n");
			return (INADDR_NONE);
		}
		if ((hostptr = gethostbyname(namestr)) == NULL) {
			fprintf(stderr, "the domain name: %s is unknown\n", namestr);
			return (INADDR_NONE);
		}
		if (hostptr->h_addrtype != AF_INET) {
			fprintf(stderr, "the domain name: %s is an unknown IPv4 address\n", namestr);
			return (INADDR_NONE);
		}
		addr_list = (struct in_addr **) hostptr->h_addr_list;
	...
	while ((c = getopt_long(argc, argv, OptionString, OptionTable, NULL)) != EOF)
	{
		switch (c)
		{
			...
			case 'a':
				if (optarg != NULL)
				{
					address.s_addr = resolve_ipv4_address();
			...
			case 'b':
				if (optarg != NULL)
				{
					broadcast_address.s_addr = resolve_ipv4_address();
			...
			case 'i':
				if (optarg != NULL)
				{
					interface.s_addr = resolve_ipv4_address();
			...
			case 'm':
				if (optarg != NULL)
				{
					multicast_group.s_addr = resolve_ipv4_address();
