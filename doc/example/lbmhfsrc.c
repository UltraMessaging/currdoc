...
"application that sends to a given topic using a single hot-failover\n"
"    source."
;
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "c:d:hi:L:l:M:P:R:s:t:Vx:")) != EOF) {
		...
		case 'x':
			bits = atoi(optarg);
			if (bits != 32 && bits != 64) {
				fprintf(stderr, "-x %s invalid, must be 32 or 64\n", optarg);
				exit(1);
			}
			break;
		default:
	...
	printf("Sending %lu messages of size %lu bytes to topic [%s]\n",
		   msgs, (unsigned long) msglen ,argv[optind]);
	current_tv(&starttv);
	if (tapeptr != NULL) {
	...
	printf("Sent %" PRIu64 " messages of size %lu bytes in %.04g seconds.\n",
			count, (unsigned long) msglen, secs);
