...
"    hot-failover receiver across contexts (HFX)."
;
...
int rcv_handle_msg(lbm_rcv_t *rcv, lbm_msg_t *msg, void *clientd)
{
	...
	case LBM_MSG_DATA:
		...
		if (opts->ascii) {
			int n = msg->len;
			const char *p = msg->data;
			printf("[%s]", opts->interfaces[(int) ((intptr_t) clientd)]);
			while (n--)
			{
				putchar(*p++);
			}
		...
		if (opts->verbose)
		{
			printf("[@%lu.%06lu]", msg->tsp.tv_sec, msg->tsp.tv_usec);
...
int main(int argc, char **argv)
{
	...
	do {
		...
		if (lbm_hfx_rcv_create(&hfxrcvs[i], hfx, ctxs[i], rattr, (void *) ((intptr_t) i)) == LBM_FAILURE) {
