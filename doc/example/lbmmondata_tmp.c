...
unsigned int RcvLBTSMXCount = 0;
unsigned int SrcLBTSMXCount = 0;
unsigned int RcvLBTRDMACount = 0;
unsigned int SrcLBTRDMACount = 0;
unsigned int RcvBROKERCount = 0;
unsigned int SrcBROKERCount = 0;
...
int main(int argc, char **argv)
{
	...
	SrcLBTSMXFile = fopen("src_lbtsmx.dat", "w");
	fprintf(SrcLBTSMXFile, "\"Timestamp\",\"Address\",\"ApplicationID\",\"Source\",");
	fprintf(SrcLBTSMXFile, "\"Clients\",\"Messages Sent\",\"Bytes Sent\"\n");

	RcvLBTSMXFile = fopen("rcv_lbtsmx.dat", "w");
	fprintf(RcvLBTSMXFile, "\"Timestamp\",\"Address\",\"ApplicationID\",\"Source\",");
	fprintf(RcvLBTSMXFile, "\"Messages Received\",\"Bytes Received\",\"LBM Messages Received\",\"Messages Received with Uninteresting Topic\",\"Requests Received\"\n");
	...
	SrcBROKERFile = fopen("src_broker.dat", "w");
	fprintf(SrcBROKERFile, "\"Timestamp\",\"Address\",\"ApplicationID\",\"Source\",");
	fprintf(SrcBROKERFile, "\"Messages Sent\",\"Bytes Sent\"\n");

	RcvBROKERFile = fopen("rcv_broker.dat", "w");
	fprintf(RcvBROKERFile, "\"Timestamp\",\"Address\",\"ApplicationID\",\"Source\",");
	fprintf(RcvBROKERFile, "\"Messages Received\",\"Bytes Received\"\n");
	...
	{
		SLEEP_SEC(2);
		printf("Source stats received... TCP: %u, LBTRU: %u, LBTRM: %u, LBTIPC: %u, LBTSMX: %u, LBTRDMA: %u BROKER: %u\n",
			   SrcTCPCount, SrcLBTRUCount, SrcLBTRMCount, SrcLBTIPCCount, SrcLBTSMXCount, SrcLBTRDMACount, SrcBROKERCount);
		printf("Receiver stats received... TCP: %u, LBTRU: %u, LBTRM: %u, LBTIPC: %u, LBTSMX: %u, LBTRDMA: %u BROKER: %u\n",
			   RcvTCPCount, RcvLBTRUCount, RcvLBTRMCount, RcvLBTIPCCount, RcvLBTSMXCount, RcvLBTRDMACount, RcvBROKERCount);
	...
	fclose(RcvLBTSMXFile);
	fclose(SrcLBTSMXFile);
	fclose(RcvBROKERFile);
	fclose(SrcBROKERFile);
	fclose(EvqFile);
	fclose(CtxFile);
