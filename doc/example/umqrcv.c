...
"application that receives brokered queuing messages from a single topic."
;
...
const char * OptionString = "AB:c:Ed:DhI:r:s:SvVX::";
...
struct Options {
	...
	char broker[256];	/* Broker address */
		...
		case LBM_TRANSPORT_STAT_LBTSMX:
			{
				fprintf(fp, " [%s] Received %lu msgs/%lu bytes. "
						...
						stats[i].source,
						stats[i].transport.lbtsmx.msgs_rcved,
						stats[i].transport.lbtsmx.bytes_rcved,
						stats[i].transport.lbtsmx.lbm_msgs_rcved,
						stats[i].transport.lbtsmx.lbm_msgs_no_topic_rcved);
			}
			break;
 */
		...
		case LBM_TRANSPORT_STAT_BROKER:
			fprintf(fp, "[%s], received %lu/%lu\n",
				stats[i].source, stats[i].transport.broker.msgs_rcved, stats[i].transport.broker.bytes_rcved);
		default:
			break;
		}
	}
	...
	case LBM_MSG_UMQ_REGISTRATION_COMPLETE_EX:
		{
			...
			printf("[%s][%s] %s \"%s\"[0x%x] registration complete. AssignID 0x%x. Flags 0x%x ", msg->topic_name, msg->source, type, reg->queue, reg->queue_id, 
	...
	case LBM_MSG_UMQ_DEREGISTRATION_COMPLETE_EX:
		{
			...
			printf("[%s][%s] %s \"%s\"[0x%x] deregistration complete. Flags 0x%x ", msg->topic_name, msg->source, type, reg->queue, reg->queue_id, reg->flags);
			printf("\n");
			close_recv = 1;
		}
		break;
	...
	case LBM_MSG_UMQ_INDEX_ASSIGNMENT_ELIGIBILITY_START_COMPLETE_EX:
		{
			...
			printf("[%s][%s] %s \"%s\"[0x%x] index assignment eligibility start complete. Flags 0x%x\n", msg->topic_name, msg->source, type, ias->queue, ias->queue_id, ias->flags);
		}
		break;
	case LBM_MSG_UMQ_INDEX_ASSIGNMENT_ELIGIBILITY_STOP_COMPLETE_EX:
		{
			...
			printf("[%s][%s] %s \"%s\"[0x%x] index assignment eligibility stop complete. Flags 0x%x\n", msg->topic_name, msg->source, type, ias->queue, ias->queue_id, ias->flags);
		}
		break;
	case LBM_MSG_UMQ_INDEX_ASSIGNED_EX:
		{
			...
			printf("[%s][%s] %s \"%s\"[0x%x] beginning of index assignment for index %s. Flags 0x%x\n", msg->topic_name, msg->source, type, ia->queue, ia->queue_id, indexstr, ia->flags);
		}
		break;
...
int main(int argc, char **argv)
{
	...
	if (opts->broker[0] != '\0') {
		if (lbm_context_attr_str_setopt(ctx_attr, "broker", opts->broker) != 0) {
	...
	if (lbm_context_attr_str_getopt(ctx_attr, "broker", opts->broker, &optlen) != 0) {
